...
def find_arbs():
    ...
    for fb in fonbet:
        ...
        fb_team1, fb_team2 = fb['match'].split(" vs ", 1)
        for pm in poly:
            if not match_events(fb['match'], pm['match']):
                continue
            pm_team1, pm_team2 = pm['match'].split(" vs ", 1)
            kf1, kf2 = fb['odds']
            kp1, kp2 = pm['odds']
            if min(kf1, kf2, kp1, kp2) < 1.15 or max(kf1, kf2, kp1, kp2) > 5.0:
                continue
            # комбинация 1: Fonbet team1 vs Polymarket team2
            p1 = profit(kf1, kp2)
            if p1 > best_profit:
                best_profit = p1
                best_kf, best_kp = kf1, kp2
                best_team_fb, best_team_pm = fb_team1, pm_team2
            # комбинация 2: Fonbet team2 vs Polymarket team1
            p2 = profit(kf2, kp1)
            if p2 > best_profit:
                best_profit = p2
                best_kf, best_kp = kf2, kp1
                best_team_fb, best_team_pm = fb_team2, pm_team1
        if best_profit <= 1:
            continue
