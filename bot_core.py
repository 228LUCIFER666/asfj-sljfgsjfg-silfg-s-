...
def match_events(fb_match, pm_match):
    ...
    if not fb1 or not pm1:
        return False
    fb1 = apply_synonyms(fb1)
    fb2 = apply_synonyms(fb2)
    pm1 = apply_synonyms(pm1)
    pm2 = apply_synonyms(pm2)
...
def find_arbs():
    ...
    try:
        ...
        log(f"Не найден модуль: {e}", "ERROR")
        return
    ...
    for fb in fonbet:
        ...
        for pm in poly:
            ...
            kf1, kf2 = fb['odds']
            kp1, kp2 = pm['odds']
            if min(kf1, kf2, kp1, kp2) < 1.2 or max(kf1, kf2, kp1, kp2) > 5.0:
                continue
            p1 = profit(kf1, kp2)
            if p1 > best_profit:
                best_profit = p1
